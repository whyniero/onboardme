...
export async function getStages(request, reply) {
    try {
...
export async function getStage(request, reply) {
    try {
...
export async function addStage(request, reply) {
    ...
    try {
        const stage = await prisma.stage.create({
            data: {
                number,
                name,
                teamleadId: teamleadId !== null && teamleadId !== void 0 ? teamleadId : null,
                internId,
                status: "NOT_STARTED",
            },
        });
...
export async function deleteStage(request, reply) {
    const user = request.user;
    if (user.role === "INTERN") {
        return reply.status(403).send({ message: "Access denied!" });
    }
...
export async function updateStage(request, reply) {
    const user = request.user;
    if (user.role === "INTERN") {
        return reply.status(403).send({ message: "Access denied!" });
    }
...
export async function getStagesForIntern(request, reply) {
    try {
