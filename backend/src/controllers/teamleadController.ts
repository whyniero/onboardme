...
export async function addTeamlead(
  request: FastifyRequest<{ Body: AddTeamleadBody }>,
  reply: FastifyReply
) {
  ...
  const { hrId, login, name, email, password, positionId, avatar } =
    request.body;
...
export async function updateTeamlead(
  request: FastifyRequest<{ Params: { id: string }; Body: UpdateTeamleadBody }>,
  reply: FastifyReply
) {
  const user = request.user as { id: string; role: string };
  if (user.role !== "HR") {
  ...
  try {
    ...
    if (positionId) dataToUpdate.positionId = positionId;
